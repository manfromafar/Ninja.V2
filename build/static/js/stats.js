...
function sharesUpdate( json ) {
	...
	if (shareInfo.block) {
		...
		for(var i=0; i<historicShares.length; i++) {
			...
			var payoutsText = (share.totalPayouts / 100000000).toFixed(0);
			
			if (share.unconfirmedPayouts) {
				payoutsText = (share.unconfirmedPayouts / 100000000).toFixed(0) + 'U + ' + payoutsText;
			}
				 
			if (share.queuedPayouts) {
				payoutsText = (share.queuedPayouts / 100000000).toFixed(0) + 'Q + ' + payoutsText;
			}
				 
			if (share.deferredPayouts) {
				payoutsText = (share.deferredPayouts / 100000000).toFixed(0) + 'D + ' + payoutsText;
			}
				 
			appendTD(entry, payoutsText );
