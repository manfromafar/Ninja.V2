...
int StaticHandler::untaint_url( std::string tainted_url ) {
	...
	std::string untainted_url = tainted_url;
	
	// CHECK: do we need to unescape URL?
	
	// CHECK: no ../ allowed
	if ( untainted_url.find( "../" ) != std::string::npos ) {
		// 400 bad request?
		return MHD_NO;
	}
	
	untainted_url = document_root + untainted_url;
	
	int res = lstat( untainted_url.c_str(), &file_sb );
	if (res == -1) {
		// couldn't stat?
