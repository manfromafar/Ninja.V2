...
// 1000ms timeout for calls to poll() to allow processing of new connections
#define WEBSOCKET_POLL_TIMEOUT 1000

// maximum size websocket backlog is allowed to grow to
#define MAX_WEBSOCKET_BACKLOG (10*1024*1024)
	...
	protected:
		...
		bool send_ws( WebSocket *ws ) {
			...
			if (unsent_buffer_length > 0) {
				// too much?
				if ( unsent_buffer_length + buffer_length > MAX_WEBSOCKET_BACKLOG ) {
					std::cout << ftime() << typeid(WebSocketHandlerEndpoint).name() << ": websocket " << sock << " unsent buffer would exceed " << MAX_WEBSOCKET_BACKLOG << " - closing" << std::endl;
					free(unsent_buffer);
					unsent_buffer = 0;
					unsent_buffer_length = 0;
	...
	private:
		...
		static void *ws_poll(void *arg) {
			...
			while(!time_to_die) {
				...
				if (nfds == 0) {
					...
					int err = pthread_cond_wait( &websocket_cond, &websocket_mutex ); 
					if ( err != 0 )
						std::cerr << ftime() << typeid(WebSocketHandlerEndpoint).name() << " pthread_cond_wait returned error code " << err << std::endl;
		...
		static void *ws_alert(void *arg) {
			...
			while(!time_to_die) {
				...
				int err = pthread_cond_wait( &alert_cond, &alert_mutex ); 
				if ( err != 0 )
					std::cerr << ftime() << typeid(WebSocketHandlerEndpoint).name() << " pthread_cond_wait returned error code " << err << std::endl;
		...
		virtual void new_websocket( void *cls, struct MHD_Connection *connection, MHD_socket sock, MHD_UpgradeActionCallback action_callback, void *upgrade_action_cls ) {
		...
		static void new_websocket_redirect( void *cls, struct MHD_Connection *connection, MHD_socket sock, MHD_UpgradeActionCallback action_callback, void *upgrade_action_cls ) {
			...
			WebSocketHandler<WebSocketHandlerEndpoint> *individual = (WebSocketHandler<WebSocketHandlerEndpoint> *)(req->handler);
			individual->new_websocket( cls, connection, sock, action_callback, upgrade_action_cls );
		}
