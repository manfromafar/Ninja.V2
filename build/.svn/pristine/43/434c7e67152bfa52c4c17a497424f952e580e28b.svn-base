...
STATIC int Handlers::API::submitNonce::inner( struct MHD_Connection *connection, Request *req, Response *resp ) {
	...
	struct sockaddr *sa = *(struct sockaddr **)(info);
	char hostname[2048] = { 0 };
	int ret = getnameinfo( sa, sizeof(*sa), hostname, sizeof(hostname), NULL, 0, 0 );
	...
	*/
	...
	if ( nonce_s.empty() ) {
		...
		std::cout << "submitNonce[" << hostname << "]: " << resp->content << std::endl;
		return MHD_YES;
	}
	...
	std::cout << "submitNonce[" << hostname << "]: account=" << req->get_query_or_post("accountId") << ", nonce=" << req->get_query_or_post("nonce") << ", deadline=" << std::to_string(deadline) << std::endl;
	...
	std::cout << "submitNonce[" << hostname << "]: OK [" << (time(NULL) - start_time) << "s] - " << deadline_string << std::endl;
